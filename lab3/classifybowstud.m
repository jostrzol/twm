...
% Wyniki dla zbioru uczącego
sel_iter = 15 % Do wypełnienia w ramach zadania 
...
% lambdas = logspace(-7, 0, 10) ; % Zagęszczenie próby...
lambdas = logspace(-7, 3, 10) ; % Zagęszczenie próby...
features = file_hist ;
features = [features, ones(size(features,1),1)] ;
for lambda = lambdas   
...
lambda = 0.00278256 % Do wypełnienia w ramach zadania
featurestrain = file_hist ;
featurestrain = [featurestrain, ones(size(featurestrain,1),1)] ;
